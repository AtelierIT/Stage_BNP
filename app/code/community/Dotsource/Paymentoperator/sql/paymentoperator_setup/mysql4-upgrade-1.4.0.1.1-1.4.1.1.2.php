...
$waitingAuth = substr('waiting_authorization_notification_paymentoperator', 0, 32);
$waitingCapture = substr('waiting_capture_notification_paymentoperator', 0, 32);
$captureReady = substr('ready_for_paymentoperator_capture_action', 0, 32);

//Update the old status to the new status to the length problems in flat_tables
$this->run("
    UPDATE {$this->getTable($orderTable)}
        SET status = 'waiting_auth_paymentoperator_note'
    WHERE
        status LIKE '$waitingAuth%';

    UPDATE {$this->getTable($orderTable)}
        SET status = 'waiting_capture_paymentoperator_note'
    WHERE
        status LIKE '$waitingCapture%';

    UPDATE {$this->getTable($orderTable)}
        SET status = 'ready_paymentoperator_capture'
    WHERE
