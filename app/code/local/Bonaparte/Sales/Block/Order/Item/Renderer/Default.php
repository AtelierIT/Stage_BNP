 ...
 */
class Bonaparte_Sales_Block_Order_Item_Renderer_Default extends Mage_Sales_Block_Order_Item_Renderer_Default
{
    ...
    public function getItemOptions()
    {
        ...
        /*
        $_product = Mage::getModel('catalog/product')->loadByAttribute('sku', $this->getItem()->getSku());
        $Sizetranslate = $_product->getBnpSizetranslate();
        $labelSize = $_product->getResource()->getAttribute('bnp_size')->getFrontendLabel();
        */

        $_product = Mage::getModel('catalog/product')->loadByAttribute('sku', $this->getItem()->getSku());

        $_product2 = $_product->getEntityId();
        $_product = Mage::getModel('catalog/product')->load($_product2);

        $Sizetranslate = $_product->getBnpSizetranslate();
        $labelSize = $_product->getResource()->getAttribute('bnp_size')->getStoreLabel();


        if($Sizetranslate != ''):
            foreach($result as $k=>$v){
                if($v['label'] == $labelSize){
                    $result[$k]['value'] = $Sizetranslate; // old value $options[$k]['value']
                }
            }
        endif;
