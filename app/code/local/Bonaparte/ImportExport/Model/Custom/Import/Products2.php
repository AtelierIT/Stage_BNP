...
//    const PICTURE_BASE_PATH = '/var/www/upload/pictures/';
     ...
     * @var string
     */
    const MISSING_PICTURES_BASE_PATH = '/dump_files/missing_pictures_part2.csv';
     ...
     * @var string
     */
    const RESOURCES_BASE_PATH = '/dump_files/tmp_import_resources_part2.csv';
    const STYLES_BASE_PATH = '/dump_files/tmp_import_styles_part2.csv';
...
//        $this->_allWebsiteIDs = Mage::getModel('core/website')->getCollection()->getAllIds();
        ...
        if ($fp = popen("mysql -u ".$config->username." -p".$config->password." ".$config->dbname." -e \"LOAD DATA LOCAL INFILE '".Mage::getBaseDir()."/dump_files/tmp_import_resources_part2.csv' REPLACE INTO TABLE bonaparte_resources FIELDS TERMINATED BY ',';\";", "r"))  {
            while( !feof($fp) ){
                echo fread($fp, 1024);
                flush();
            }
        ...
        if ($fp = popen("mysql -u ".$config->username." -p".$config->password." ".$config->dbname." -e \"LOAD DATA LOCAL INFILE '".Mage::getBaseDir()."/dump_files/tmp_import_styles_part2.csv' REPLACE INTO TABLE bonaparte_styles FIELDS TERMINATED BY ',';\";", "r"))  {
            $this->_logMessage('Done!' . "\n");
            while( !feof($fp) ){
                echo fread($fp, 1024);
                flush();
